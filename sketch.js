...
function setup() {
    ...
    final = createSprite(1452, 92, 20, 20)
...
function draw() {
    ...
    if (keyWentDown("space")){
        Spc.changeAnimation("chute", Chute);

    }

    if (Spc.isTouching(final)){
        Spc.x = 1452
        Spc.y = windowHeight - 220
        Pontos_Final = Ponto_Final + 1
    }


    if (frameCount % 130 === 0) {
        inimigo = createSprite(570, 698);
        inimigo2 = createSprite(960, 400);
        inimigo3 = createSprite(388, 110);
